...
def seu_login(
    ...
    while captcha_required and not captcha_correct:
        ...
        captcha_result = (
            solve_captcha(session, ocr, manual_captcha) if captcha_required else ""
        )
...
if __name__ == "__main__":
    ...
    session, redirect_url = seu_login(
        username, password, service_url, ocr=ocr, manual_captcha=False
    )
    if not session:
        exit(1)
    ...
    if redirect_url:
        ...
        res = session.get(
            url=f"http://ehall.seu.edu.cn/jsonp/userDesktopInfo.json?type=&_={int(datetime.now().timestamp())}",
            verify=False,
        )
        print(res.json())
